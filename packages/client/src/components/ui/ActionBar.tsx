    ...
    return (
      <div style={styles.bar}>
        ...
        <div style={styles.btnRow}>
          <button style={{ ...styles.btn, opacity: selectedForSwap.length === 0 ? 0.5 : 1 }} onClick={swapTiles} disabled={selectedForSwap.length === 0}>
    ...
    <div style={styles.bar}>
      ...
      <div style={styles.btnRow}>
        {pendingPlacements.length > 0 ? (
          <>
            <button
              style={{ ...styles.btn, background: previewValid ? '#5E7A52' : '#C8B896', color: previewValid ? '#fff' : '#A89878', border: previewValid ? '2px solid #4A6040' : '2px solid #A89878', boxShadow: previewValid ? '2px 2px 0px #4A6040' : 'none' }}
              onClick={submitMove}
              disabled={!previewValid}
            >
              Submit
            </button>
            <button style={styles.btnSecondary} onClick={undoAll}>
              Undo
            </button>
          </>
        ) : (
          <>
            <button style={styles.btnSecondary} onClick={skipTurn}>
              Skip
            </button>
            <button
              style={styles.btnSecondary}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  hint: { color: '#A89878', fontSize: '0.75rem' },
  preview: { },
  previewScore: { color: '#5E7A52', fontWeight: 700, fontSize: '1rem' },
  previewInvalid: { color: '#A75B47', fontSize: '0.78rem', lineHeight: 1.3 },
